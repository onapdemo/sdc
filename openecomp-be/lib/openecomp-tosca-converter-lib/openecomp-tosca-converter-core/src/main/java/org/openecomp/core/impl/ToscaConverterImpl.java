...
public class ToscaConverterImpl implements ToscaConverter {
    ...
    private void convertTopologyTemplate(ServiceTemplate serviceTemplate,
        ...
        convertGroups(serviceTemplate, readerService);
    }

    private void convertGroups(ServiceTemplate serviceTemplate,
                               ServiceTemplateReaderService readerService) {
        Map<String, Object> groups = readerService.getGroups();
        if (MapUtils.isEmpty(groups)) {
            return;
        }

        for (Map.Entry<String, Object> groupEntry : groups.entrySet()) {
            Optional<GroupDefinition> groupDefinition =
                ToscaConverterUtil.createObjectFromClass(groupEntry.getKey(), groupEntry.getValue(),
                    GroupDefinition.class);
            DataModelUtil.addGroupDefinitionToTopologyTemplate(serviceTemplate,groupEntry.getKey(),
                groupDefinition.get());
        }
    }
